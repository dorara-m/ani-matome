...
- メニューを sp 用につくる
- 各アニメの下層でも遷移できるようにヘッダー端になんかおく → いっそ sp と共通化でも？
...
- [!!] キャラ追加みたいなのができたら、それの人数を cms で自動カウントして返してくれるようにしたい。
...
- 未読のフラグはローカルストレージを使う。ページアクセス時に全チャットに対して未読フラグを付与しておき、ユーザーが開いたらフラグを反転。
  ...
  - [バグ][!!] サイトの初回ロード時に localStorage 読むより前に以降の処理が走ってしまってエラーになる。→ ローカルストレージ読み込みを待つような処理。
  - [バグ] json で画像がチャットの最後になってた場合、それを処理できず makelisthtml が止まる。
    -> 除外処理をいれる
...
- [!!!] 一覧の頭にストーリー機能で、キャラ自身を掘れるようにしておく。→ 現状の UI だとストーリーは余分かも。
  -----対応予定-----
