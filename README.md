...
* ブラウザ指定からIEをとる。
* 更新が楽しくなる仕組み。
  * オグリとタマのやり取りがついでに更新される。チャットで会話してる風。
  * サイトトップを開いたときにふわっと要素が出る。ずっと表示させるわけにもいかない、基本3日とかで消えるように。
  * 何度も見たいので、ログとしてどこかに入るようにしておく。
  * 更新しやすさを考えると、特定のセリフをjson（releasenoteとか）追記すれば勝手にその内容を読んで表示、時間たったらログに移動されるようにしておきたい。
### コンテンツ編
