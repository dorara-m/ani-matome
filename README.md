...
- [Middle] カテゴリーで絞り込み可能にする。
- [Middle] サイト全体のリデザイン。sp でのテーブルなどいけてないので、モバイルファーストな見た目に。c-btn が見た目好きじゃないので調整
- [Light] api キーを直で書いちゃってる問題。.env ファイルにきりわけて git にのせないようにする。
- [Light] 画面遷移したときに馬チャットがアニメーションする謎。
- [Light] microCMS で取得する画像サイズを抑えるように調整（某案件参考
- [Light] ブラウザ指定で調査中にしてるやつ、一気に埋める。
- [Light] 関連リンクとかをちゃんと記載してないやつ、整理
- [Middle] 各サンプルのソースを表に出す。コピペ機能もつけたい。
- [Heavy] サイトから更新通知を出す。プッシュ？RSS？まだイメージついてない。

### アニメ単体の改修

- [!!!] 星空のアニメーションがビルドするたびに差分が出てしまう問題 → scss をランダム生成を off
...
- https://photoshopvip.net/121220#tip7
  「ろってぃぷれびゅー」
- ウマ娘でつかう予定のチャットしてる風アニメ（参考, peep アプリ
- [!!] 動画の読み込み速度比較, youtube,video タグ,vimeo

### gulp など環境にまつわるもの（gulp を捨てる可能性高いので優先度最低）

- [Light] 共通 css ファイルが細分化しすぎてる問題。common.scss とか 1 つにまとめたい。
- [Light] af で作ったものを youtube にあげるので、それを自動取得したい。youtube の API つかえそう。
- [Light] pug の mixin がコンパイルエラーになる問題。in pageContent.pug
- [Heavy] ページごとの scss ファイルを自動でつくる？（Next 化すればいらないかも
- [?] cms で html を更新するのがやりづらい問題（cms での運用上、避けられないのでは、、見積もり不可
